...
class TelegramBot:
    ...
    async def pull(self, update: Update, context: CallbackContext, level_range: Tuple[int, int]):
        ...
        for domain in domains:
            for msg in self.messages_to_log(domain=domain, cid=chat_id, level_range=level_range, past=past, repeat=True):
    ...
    async def cron(self, context: ContextTypes.DEFAULT_TYPE, domain: int | str, cid: int, level: int, freq: int):
        for msg in self.messages_to_log(domain=domain, cid=cid, level_range=(level, 100), past=max(10, freq*10), repeat=False):
